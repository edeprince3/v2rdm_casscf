...
namespace v2rdm_grad {
  ...
  void v2RDMSolver::jk_common_init()
  {
...
SharedMatrix v2RDMSolver::jk_compute_gradient()
{
  jk_common_init();
    // => Echo <= //
    ...
    SharedMatrix Dav(new Matrix("Alpha MO Coefficients", nmo_, nmo_));
    SharedMatrix TPDM(new Matrix("Alpha MO Coefficients", nmo_*nmo_, nmo_*nmo_));
    SharedMatrix Dbv(new Matrix("Beta MO Coefficients", nmo_, nmo_));
    SharedMatrix opdm_a=SharedMatrix(Da_->clone());
    SharedMatrix opdm_b=SharedMatrix(Db_->clone());
    opdm_a->zero();
    opdm_b->zero();
	...
	if (nalpha_!=nbeta_){
	outfile->Printf("This only works for nalpha_ == nbeta_\n");
	exit(0);
	}
	...
	for (int j = 0; j < nalpha_; j++) {
	  for (int k = 0; k < nalpha_; k++) {
	    for (int l = 0; l < nalpha_; l++) {
	      TPDM->pointer()[i*nmo_+k][j*nmo_+l] = 0.5*(double)((i==k)*(j==l)-(j==k)*(i==l));
	      //	    outfile->Printf("%3i %3i %3i %3i :  %12.5lf \n",i, j, k,l,0.5*(double)((i==k)*(j==l)-(j==k)*(i==l)));
	    }
	  }
	}
      }
    ...
    //Dav->remove_symmetry(opdm_a,sotoao);
    
    Da = Da_subset("AO");
    // Da->add(Dav);
    //Da =Dav;
    // outfile->Printf("N alpha: %12.5lf, N beta: %12.5lf ", Da->trace(), Dbv->trace());
     if (options_.get_str("REFERENCE") == "RHF") {
       // Dbv->remove_symmetry(opdm_b,sotoao);
       Db = Da;
       //Dt->add(Db);
       //Db->subtract(Dbv);
     } else {//Not RHF
    ...
    // outfile->Printf("just getting to density \n");
    Catemp = Ca_subset("AO", "OCC");
    Ca = Ca_subset("AO");
    eps_a = epsilon_a_subset("AO", "OCC");
    //eps_a = epsilon_a_subset("AO");
    // if (options_.get_str("REFERENCE") == "RHF" ) {
    //    Cb = Ca;
    eps_b = eps_a;
    // } else {//not RHF
    Cbtemp = Cb_subset("AO", "OCC");
    Cb = Cb_subset("AO");
    ...
    int nalpha = nalpha_;//Ca->colspi()[0];
    int nbeta = nbeta_;//Cb->colspi()[0];
    ...
    timer_on("Grad: S");
    {
        ...
        double** Wp = W->pointer();
        double** Cap = Catemp->pointer(); 
        double** Cbp = Cbtemp->pointer(); 
    ...
    gradients["Metric"] = jk_gradients["Metric"];
    gradients["3index"] = jk_gradients["3index"];
    
    // gradients["Exchange"]->scale(-1.0);
    timer_off("Grad: JK");
    //gradients["Coulomb"]->print_atom_vector(); 
    // gradients["Exchange"]->print_atom_vector();
