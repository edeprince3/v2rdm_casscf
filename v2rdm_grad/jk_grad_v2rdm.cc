...
namespace psi {
  namespace v2rdm_grad {
    ...
    void DFJKGrad::compute_gradient()
    {
      ...
      gradients_["Metric"]= SharedMatrix(new Matrix("Metric Gradient",natom,3));
      gradients_["3index"]= SharedMatrix(new Matrix("3index Gradient",natom,3));
      // => Build ERI Sieve <= //
     ...
     *          form J(P,Q)^-1/2
     =======================================================*/          
    void DFJKGrad::formJ(boost::shared_ptr<BasisSet> auxiliary_, boost::shared_ptr<BasisSet> zero)
    {
    ...
    void DFJKGrad::form_CQij(){ 
     // => Sizing <= //
      ...
      int na = Ca_->colspi()[0];
      int nb = Cb_->colspi()[0];
      double** Ap = block_matrix(naux, nso*nso); 
      double** Bp = block_matrix(naux, nso*nso);
      double** Cap =Ca_->pointer();
      double** Cbp =Cb_->pointer();
      double ** BJmhalf = block_matrix(naux, nso*nso); 
      SharedTensor2d Amn = SharedTensor2d(new Tensor2d("B(Q|mn) ",naux, nso*nso));
      CQmn = SharedTensor2d(new Tensor2d("C(Q|mn) ",naux, nso*nso)); //so
      CQpq = SharedTensor2d(new Tensor2d("C(Q|pq) ",naux, nso*nso)); //mo
...
#pragma omp parallel for schedule(dynamic) num_threads(nthreads)
	  ...
	  for (int p = 0; p < nP; p++) {
		...
		Bp[p + oP][(m + oM) * nso + (n + oN)] = buffer[thread][index]; 
		Bp[p + oP][(n + oN) * nso + (m + oM)] = buffer[thread][index]; 
	      }
	    }
	  }
	}
    ...
    Amn->set(Bp);
    C_DGEMM('N','N', naux, nso*nso, naux, 1.0, J_mhalf[0], naux, Bp[0], nso*nso, 0.0, Ap[0], nso*nso);
	...
	SharedTensor2d cQnoA = SharedTensor2d(new Tensor2d("DF_BASIS_SCF C (Q|mO)", naux, nso * na)); 
	SharedTensor2d cQooA = SharedTensor2d(new Tensor2d("DF_BASIS_SCF C (Q|OO)", naux, na * na));
	SharedTensor2d CmoA = SharedTensor2d(new Tensor2d("Alpha MO Coefficients", nso, na)); //need to be removed
        
	// RHF
	// Build CmoA 
	for (int mu = 0; mu < nso; mu++) {
	  for (int i = 0; i < na; i++) {
	    CmoA->set(mu, i, Ca_->get(mu, i));
	  }
	}
	cQnoA->contract(false, false, naux * nso, na, nso, CQmn, CmoA, 1.0, 0.0);
	...
	GQpq = SharedTensor2d(new Tensor2d(" G (Q|pq)", naux, nso*nso));
	GQmn = SharedTensor2d(new Tensor2d(" G (Q|mn)", naux, nso*nso));
	GPQ = SharedTensor2d(new Tensor2d("2-index TPDM G(P|Q)", naux, naux));
	
	SharedTensor2d gQon_ref = SharedTensor2d(new Tensor2d("DF_BASIS_SCF G_imu^Q", naux, nso * na));
	gQon_ref->contract(false, true, naux * na, nso, na, GQpq_ref, CmoA, 1.0, 0.0);
	GQmn->contract233(false, false, nso, nso, CmoA, gQon_ref, 1.0, 0.0);
	...
	GQpq->zero();//initialize for correlation type
	GPQ->zero(); //initialize for correlation type
	
	
	//For now compute density over ocuppied MO
	SharedTensor2d G_ref = SharedTensor2d(new Tensor2d(" G temp", na*na, na*na));
	SharedMatrix Ga = SharedMatrix(new Matrix(" Da in MO basis", nso, nso));
	SharedMatrix Gb = SharedMatrix(new Matrix(" Db in MO basis", nso, nso));
	Ga->transform(Da_,Ca_);
	Gb->transform(Db_,Cb_);
	
	
	double** Dap = Ga->pointer();
	double** Dbp = Gb->pointer();
	
	
	double** G_refp = block_matrix(na*na, na*na);
	
       	for(int p=0;p<na;p++){      
	  for(int q=0;q<na;q++){
	    for(int r=0;r<na;r++){      
	      for(int s=0;s<na;s++){      
		double dab = Dap[p][r]*Dbp[q][s];
		double daa = Dap[p][r]*Dap[q][s] - Dap[p][s]*Dap[q][r];
		double dbb = Dbp[p][r]*Dbp[q][s] - Dbp[p][s]*Dbp[q][r];
		G_refp[p*na+r][q*na+s]= daa + dbb+ 2.0*dab ;
	      }
	    }
	  }
	}
	...
	cQnoA->contract(false, false, naux * nso, na, nso, CQmn, CmoA, 1.0, 0.0);
	cQooA->contract233(true, false, na, na, CmoA, cQnoA, 1.0, 0.0);
	SharedTensor2d GQpq_ref = SharedTensor2d(new Tensor2d("DF_BASIS_SCF G_imu^Q", naux, na * na));
	cQooA->scale(2.0);
	
	GQpq_ref->gemm(false, false,cQooA, G_ref,1.0,0.0);

	/*GQpq_ref->zero();
	for (int Q = 0; Q < naux; Q++) {
	// for(int i = 0; i < na*na; ++i) {
	  for(int j = 0; j < na*na; ++j) {
	    double summ = 0.0;
	    for(int k = 0; k < na*na; ++k) {
	      //	for(int l = 0; l < na; ++l) {
	      summ +=  cQooA->get(Q,k)*G_ref->get(k,j);
	      //	}
	    }
	    GQpq_ref->set(Q,j,summ);
	  }
	  //  }
	  }
	  outfile->Printf("rms GQpq %12.5lf\n",GQpq_ref->rms());*/
	...
	GQmn->scale(2.0);
	GPQ->gemm(false,true, cQooA, GQpq_ref, 1.0, 0.0);// MO basis 
	outfile->Printf("MO rms GPQ %12.5lf\n",GPQ->rms());
	GPQ->zero();
	GPQ->gemm(false,true, CQmn, GQmn, 1.0, 0.0);// MO basis 
	outfile->Printf("AO rms GPQ %12.5lf\n",GPQ->rms());
	outfile->Printf("rms GQmn %12.5lf\n",GQmn->rms());
     ...
     * build G_PQ J_PQ
     ====================================================================*/
    void DFJKGrad::build_metric_deriv()
    {
    ...
    for (int t = 0; t < df_ints_num_threads_; t++) {
      gradients_["Metric"]->add(Ktemps[t]);
    }
    gradients_["Metric"]->print_atom_vector(); 
    }

    /*===================================================================
     * build G^Q_pq (Q|pq)
     ====================================================================*/
    void DFJKGrad::build_3index_deriv()
    {
      // => Sizing <= //
      ...
      int nso = primary_->nbf();
      int naux = auxiliary_->nbf();
      double** GQpq_p = GQmn->to_block_matrix();
	...
	for (long int PMN = 0L; PMN < NP * npairs; PMN++) {
	  ...
	  for (int p = 0; p < nP; p++) {
	    for (int m = 0; m < nM; m++) {
	      for (int n = 0; n < nN; n++) {
		double Jval = perm * GQpq_p[p + oP][(m + oM) * nso + (n + oN)];
      ...
      for (int t = 0; t < df_ints_num_threads_; t++) 
	gradients_["3index"]->add(Ktemps[t]);

      gradients_["3index"]->print_atom_vector();
    }
...
void DFJKGrad::build_Amn_x_terms()
{
    ...
    // outfile->Printf("norm J %12.5lf",d->norm());
    SharedMatrix Jmn;
    SharedMatrix Kmn;
    SharedMatrix Ami;
    SharedMatrix Aij;
	    ...
	    for (int p = 0; p < nP; p++) {
                for (int m = 0; m < nM; m++) {
                    for (int n = 0; n < nN; n++) {
                        ...
                        if (do_J_) {
			  double Ival =  perm * dp[p + oP + pstart]* Dtp[m + oM][n + oN];
                        ...
                        if (do_K_) {
                            double Jval = perm * Jmnp[p + oP][(m + oM) * nso + (n + oN)];
                            grad_Kp[aP][0] += Jval * (*Px);
                            grad_Kp[aP][1] += Jval * (*Py);
                            grad_Kp[aP][2] += Jval * (*Pz);
                            grad_Kp[aM][0] += Jval * (*Mx);
