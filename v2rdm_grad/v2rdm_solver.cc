...
static void evaluate_Ap(long int n, SharedVector Ax, SharedVector x, void * data) {
    ...
    v2rdm_grad::v2RDMSolver* BPSDPcg = reinterpret_cast<v2rdm_grad::v2RDMSolver*>(data);
...
double v2RDMSolver::compute_energy() {
    ...
    boost::shared_ptr<CGSolver> cg (new CGSolver(N));
    ...
    //analytical gradient initiation 
    outfile->Printf("     starting integral derivative \n");
    jk_compute_gradient();
    outfile->Printf("     exiting integral derivative \n");
    exit(0);
    int oiter=0;
    do {
